...
def twoway_admixture_demo(t1, t2, r, N, N_a, N_b, N_c, m_ab):
  ...
  demo.add_phase(ContinuousPhase(t1, 100, Q = np.array([[-m_ab, m_ab], [m_ab, -m_ab]]), n = [1/N_a, 1/N_b]))
  demo.add_phase(ContinuousPhase(100, t2, Q = np.array([[0, 0], [0, 0]]), n = [1/N_a, 1/N_b]))
  demo.add_phase(DiscretePhase(t2, P = np.array([[1], [1]])))
  demo.add_phase(ContinuousPhase(t2, math.inf, Q = np.array([[0]]), n = [1/N_c]))
  return demo


def twoway_admixture_demography(t1, t2, r, N, N_a, N_b, N_c, m_ab):
  demography = msprime.Demography()
  demography.add_population(name = "O", initial_size = N)
  demography.add_population(name = "A", initial_size = N_a)
  demography.add_population(name = "B", initial_size = N_b)
  ...
  demography.add_symmetric_migration_rate_change(time=t1, populations = ["A", "B"], rate = m_ab)
  demography.add_symmetric_migration_rate_change(time=100, populations = ["A", "B"], rate = 0)
