...
def estimate(trees, model, search, samples = None, prune = 0):
  x = search.get()
  logp_max = glike_trees(trees, model(*x), samples = samples, prune = 0)
  ...
  for _ in range(100):
    for name in [name for name in search.names if name not in search.names_fixed]:
      x_up = search.up(name)
      logp_up = glike_trees(trees, model(*x_up), samples = samples, prune = 0)
      x_down = search.down(name)
      logp_down = glike_trees(trees, model(*x_down), samples = samples, prune = 0)
